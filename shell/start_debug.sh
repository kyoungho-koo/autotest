echo > /var/log/syslog
echo > /var/log/kern.log
#echo 0 > /sys/module/jbd2/parameters/jbd2_debug
#cat /sys/module/jbd2/parameters/jbd2_debug


#
echo 0 > /proc/sys/kernel/randomize_va_space
cat /proc/sys/kernel/randomize_va_space

ls /home/dertflag/Templates/workload
read workload

#umount /home/dertflag/mnt
#mkfs.ext4 -E lazy_itable_init=0,lazy_journal_init=0 /dev/nvme0n1p1
#mkfs.ext4 -E lazy_itable_init=0,lazy_journal_init=0 /dev/sdb1 

#mount -o data=journal /dev/nvme0n1p1 /home/dertflag/mnt
#mount -o data=journal /dev/sdb1 /home/dertflag/mnt
#mount /dev/nvme0n1p1 /home/dertflag/mnt
#mount /dev/sdb1 /home/dertflag/mnt

#free all data in memory
free -h && sync && sh -c 'echo 3 > /proc/sys/vm/drop_caches' && free -h

filebench -f /home/dertflag/Templates/workload/$workload.f
