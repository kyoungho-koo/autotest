...
OUTDIR="$(pwd)/output/sysbench-ext4"
...
do
  ...
  #free all data in memory
    ...
    sysbench --test=fileio --file-total-size=20G --file-test-mode=rndrw --file-fsync-all=on --num-threads=$i --max-time=60 --max-requests=0 run >> ${OUTDIR}/${STORAGE}thread$i;
